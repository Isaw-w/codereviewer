...
def setup_client(api_provider, api_key):
    ...
    if api_provider == 'openai':
        ...
        # Avoid indefinite hangs on long-tail requests during resume runs.
        # Keep timeout configurable without code edits.
    ...
    elif api_provider == 'furion':
        # Furion can occasionally hang on long generations; enforce a client-side request timeout.
        # Allow override via env for incident-time tuning without code edits.
        timeout_sec = int(os.getenv("FURION_REQUEST_TIMEOUT_SEC", "45"))
        return OpenAI(
            api_key=api_key,
...
def _chat_create_with_top_p_fallback(client, params):
    """
    Some upstreams (e.g., Furion Claude/Bedrock routes) reject requests that set
    both `temperature` and `top_p`. Keep default behavior first, then retry once
    without `top_p` only for that specific validation error.
    """
    try:
        ...
        if "temperature" in msg and "top_p" in msg and "cannot both" in msg:
            params_no_top_p = dict(params)
            params_no_top_p.pop("top_p", None)
            return client.chat.completions.create(**params_no_top_p)
        raise
...
def _row_counts_as_completed(row):
    if "model_resp" not in row:
        if "judgement" in row:
            judgement = row.get("judgement")
            if judgement is None:
                return False
            if isinstance(judgement, str):
                return bool(judgement.strip())
            return True
        return True
        ...
        with open(output_file, 'r') as f:
            ...
            for line in f:
                if not line.strip():
                    continue
                row = json.loads(line)
                if _row_counts_as_completed(row):
                    out.append(row['idx'])
            return out
...
def collect_response(client, model, user_prompt, api_provider):
    ...
    if api_provider in ['openai', 'openrouter', 'togetherai', 'xai', 'furion']:
        completion = _chat_create_with_top_p_fallback(client, params)
        return (
...
def collect_thinking_response(client, model, user_prompt, api_provider, budget_tokens, reasoning_effort):
    ...
    if api_provider == 'anthropic':
        ...
        return _collect_openrouter_thinking(client, model, message_prompts, budget_tokens, reasoning_effort)
...
def _collect_openrouter_thinking(client, model, message_prompts, budget_tokens, reasoning_effort):
...
def get_judge_response(client, model, prompt, temperature=1, top_p=1, max_tokens=10500, reasoning_effort="medium"):
...
def calculate_task_level_averages(task_id_to_criteria, field):
    ...
    if field == "len":
        values = [
            len(criteria[0]["response"]) if criteria[0]["response"] is not None else 0
