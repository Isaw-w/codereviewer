...
The fetch wrapper writes to `./data/` and refuses to overwrite if `./data/` is already populated; pass `--force` to re-fetch. By default, it pins `HF_REVISION` to `c165536f6b4393df4d71e3897bc5f1fb23f6e4b5`, the verified dataset commit containing the complete Finding 2 and Finding 3 release data. Set `HF_REVISION=main` only if you intentionally want the latest dataset state. After this step, `data/paper_release/` and `data/canonical_full/` should exist as siblings of `code/`.
